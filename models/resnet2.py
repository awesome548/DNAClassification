...
class block(nn.Module):
    def __init__(self, first_in_channels, first_out_channels, identity_conv=None, stride=1):
        """
        残差ブロックを作成するクラス
        Args:
        ...
        # 1番目のconv層（1×1）
        self.conv1 = conv_1(first_in_channels,first_out_channels)
        self.bn1 = bcnorm(first_out_channels)

        # 2番目のconv層（3×3）
        # パターン3の時はsizeを変更できるようにstrideは可変
        self.conv2 = conv_3(first_out_channels,first_out_channels,stride=stride)
        self.bn2 = bcnorm(first_out_channels)

        # 3番目のconv層（1×1）
        # output channelはinput channelの4倍になる
        self.conv3 = conv_1(first_out_channels,first_out_channels*4)
        self.bn3 = bcnorm(first_out_channels*4)
        self.relu = nn.ReLU()

        # identityのchannel数の調整が必要な場合はconv層（1×1）を用意、不要な場合はNone
        self.identity_conv = nn.Conv1d(first_out_channels) 
